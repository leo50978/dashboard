...
async function fetchRemoteRuntimeConfigViaHttp() {
  const apiBaseUrl = String(dashboardRuntimeConfig.apiBaseUrl || "").trim().replace(/\/+$/, "");
  if (!apiBaseUrl) {
    throw new Error("API runtime non configuree.");
  }

  const controller = typeof AbortController !== "undefined" ? new AbortController() : null;
  const timer = controller
    ? window.setTimeout(() => controller.abort(), DEFAULT_RUNTIME_CONFIG_HTTP_TIMEOUT_MS)
    : 0;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(String(payload?.message || "Configuration runtime indisponible."));
    }
...
async function bootstrapRemoteAppCheck() {
  ...
  appCheckBootstrapPromise = (async () => {
    try {
      const payload = await fetchRemoteRuntimeConfig();

...
function resolveRequestedRange() {
  ...
  if (!startRaw || !endRaw) {
    return {
      ok: false,
      message: "Choisis une date de debut et une date de fin pour utiliser une periode personnalisee.",
    };
  }
  ...
  if ((endMs - startMs) > maxRangeMs) {
    return { ok: false, message: "La periode est trop large. Choisis maximum 180 jours." };
  }
...
function renderList(node, items = [], fallback = "Aucun point a afficher.") {
  if (!node) return;
...
function renderHealth(snapshot = {}) {
  ...
  if (dom.healthCopy) {
    ...
    dom.healthCopy.textContent = weakest
      ...
      : "Le score apparaitra ici apres la premiere lecture.";
  }
  if (dom.generatedAt) {
...
function renderMetrics(snapshot = {}) {
  ...
  dom.metrics.innerHTML = `
    <article class="neo mini-card">
      <p class="metric-label">Trafic et presence</p>
      <p class="metric-value">${formatInt(visits.rangeVisits)}</p>
      <p class="metric-note">
        Visites sur la fenetre. Pic visiteurs: ${formatInt(presence.peakVisitors)}. Pic joueurs: ${formatInt(presence.peakPlayers)}.
      </p>
    </article>
    <article class="neo mini-card">
      <p class="metric-label">Parties terminees</p>
      <p class="metric-value">${formatInt(games.totalMatches)}</p>
      <p class="metric-note">
        Domino ${formatInt(games.classicMatches)} | Duel ${formatInt(games.duelMatches)} | Morpion ${formatInt(games.morpionMatches)} | Dame ${formatInt(games.dameMatches)} | Pong ${formatInt(games.pongMatches)}.
      </p>
    </article>
    <article class="neo mini-card">
    ...
    <article class="neo mini-card">
      ...
      <p class="metric-note">
        Inscriptions sur la fenetre. Conversion inscription vers depot: ${formatPct(acquisition.signupToDepositRatePct)}.
      </p>
    </article>
    <article class="neo mini-card">
...
function buildPromptFacts(snapshot = {}) {
  ...
  return {
    ...
    games: {
      ...
      dameMatches: safeInt(games.dameMatches),
      pongMatches: safeInt(games.pongMatches),
      ...
      dameWithBots: safeInt(games.dameWithBots),
      pongWithBots: safeInt(games.pongWithBots),
...
function renderSnapshot(snapshot = {}) {
  ...
  renderList(dom.strengths, snapshot.narrative?.strengths, "Aucun point fort remonte.");
  renderList(dom.alerts, snapshot.narrative?.alerts, "Aucune alerte remontee.");
  renderList(dom.priorities, snapshot.narrative?.priorities, "Aucune priorite remontee.");
  renderPrompt(snapshot);
}

async function generateReport() {
  try {
    setStatus("Generation du rapport conseiller IA en cours...", "warning");
    if (dom.generateBtn) dom.generateBtn.disabled = true;
    await ensureFinanceDashboardSession({
      title: "Conseiller IA",
      description: "Connecte-toi avec le compte administrateur autorise pour generer une lecture executive du site.",
    });

    const reportType = String(dom.reportType?.value || "daily").trim().toLowerCase() === "global"
      ? "global"
      : "daily";
    ...
    if (!snapshot) {
      throw new Error("Snapshot conseiller IA introuvable.");
    }
    ...
    console.error("[AI_ADVISOR_V2] generate error", error);
    setStatus(error?.message || "Impossible de generer le rapport conseiller IA.", "error");
  } finally {
...
async function copyPrompt() {
  try {
    ...
    console.error("[AI_ADVISOR_V2] copy error", error);
...
function refreshReadyStatus() {
  ...
  if (period.hasRange) {
    setStatus(`Mode pret: ${nextLabel} | Periode: ${period.label}. Clique sur "Generer le rapport".`, "neutral");
    return;
  }
  setStatus(`Mode pret: ${nextLabel}. Clique sur "Generer le rapport" pour lancer les lectures utiles.`, "neutral");
}
...
function bootstrap() {
  if (dom.promptOutput) {
    dom.promptOutput.value = "Le prompt sera genere ici.";
  }
  if (dom.promptMeta) {
    dom.promptMeta.textContent = "Aucune periode active pour l'instant.";
  }
  refreshReadyStatus();
  bindEvents();
}

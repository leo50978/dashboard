  ...
  overlay.innerHTML = `
    <div style="width:100%;max-width:28rem;border-radius:1.5rem;border:1px solid rgba(255,255,255,0.12);background:rgba(2,6,23,0.92);padding:1.5rem;color:#fff;box-shadow:0 24px 80px rgba(0,0,0,0.55);font-family:inherit;">
      ...
      <form id="sharedDashboardAuthForm">
        ...
        ${showConfirmPassword ? `
        <div style="display:grid;gap:0.35rem;margin-bottom:1rem;">
          <label style="font-size:0.9rem;font-weight:600;color:#e2e8f0;" for="sharedDashboardAuthPasswordConfirm">Confirme mot de passe</label>
          <input
            id="sharedDashboardAuthPasswordConfirm"
            type="password"
            style="width:100%;border-radius:1rem;border:1px solid rgba(255,255,255,0.12);background:#0f172a;color:#fff;padding:0.9rem 1rem;outline:none;"
            placeholder="Confirme mot de passe la"
          />
        </div>
        ` : ""}
        ...
        <button
          ...
          ${busy ? "disabled" : ""}
        >${busy ? "Traitement..." : escapeHtml(submitLabel)}</button>
      </form>
    </div>
  `;
...
async function bootstrapFirstAdmin(email, user) {
  await setDoc(
    doc(db, "settings", BOOTSTRAP_DOC_ID),
    {
      email,
      uid: String(user?.uid || ""),
      mode: "email_password",
...
async function promptAdminLogin(expectedEmail = "", context = {}) {
  return new Promise((resolve, reject) => {
    const isBootstrapMode = !expectedEmail;
    const contextTitle = context.title || "Dashboard";
    const description = isBootstrapMode
      ? "Aucun compte admin autorise pa egziste toujou sou nouvo base la. Kreye premye kont admin lan kounye a."
      : (context.description || "Connecte-toi avec le compte administrateur autorise pour acceder a ce dashboard.");
    ...
    const render = (error = "", busy = false) => {
      const overlay = renderOverlay({
        ...
        error,
        busy,
        submitLabel: isBootstrapMode ? "Kreye premye admin lan" : "Se connecter",
        showConfirmPassword: isBootstrapMode,
      });
      ...
      overlay.querySelector("#sharedDashboardAuthForm")?.addEventListener("submit", async (event) => {
        ...
        const passwordConfirm = String(
          overlay.querySelector("#sharedDashboardAuthPasswordConfirm")?.value || ""
        );

        if (!email || !password) {
          render("Entre email la ak modpas la.");
          return;
        }
        if (isBootstrapMode && password.length < 6) {
          render("Mot de passe a dwe genyen omwen 6 karakte.");
          return;
        }
        if (isBootstrapMode && password !== passwordConfirm) {
          render("De modpas yo pa menm.");
          return;
        }
        ...
        try {
          let credentials;
          if (isBootstrapMode) {
            credentials = await signupWithEmail(email, password);
            await bootstrapFirstAdmin(email, credentials.user);
          } else {
            credentials = await loginWithEmail(email, password);
          }
          hideOverlay();
          resolve(credentials.user);
        } catch (error) {
          const fallback = isBootstrapMode
            ? "Creation du premier admin impossible."
            : "Connexion impossible.";
          render(formatAuthError(error, fallback));
        }
      });
    };
...
export async function ensureFinanceDashboardSession(options = {}) {
  ...
  if (!isAllowedAdminUser(user, expectedEmail)) {
    await logoutCurrentUser().catch(() => {});
    throw new Error("Ce compte n'est pas autorise pour ce dashboard.");
  }

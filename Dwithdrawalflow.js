...
const dom = {
  adminEmail: document.getElementById("withdrawalFlowAdminEmail"),
  windowBadge: document.getElementById("withdrawalFlowWindowBadge"),
  scanBadge: document.getElementById("withdrawalFlowScanBadge"),
  dateFrom: document.getElementById("withdrawalFlowDateFrom"),
  dateTo: document.getElementById("withdrawalFlowDateTo"),
  granularity: document.getElementById("withdrawalFlowGranularity"),
  refreshBtn: document.getElementById("withdrawalFlowRefreshBtn"),
  status: document.getElementById("withdrawalFlowStatus"),
  requestedValue: document.getElementById("withdrawalFlowRequestedValue"),
  requestedCopy: document.getElementById("withdrawalFlowRequestedCopy"),
  approvedValue: document.getElementById("withdrawalFlowApprovedValue"),
  approvedCopy: document.getElementById("withdrawalFlowApprovedCopy"),
  rejectedValue: document.getElementById("withdrawalFlowRejectedValue"),
  rejectedCopy: document.getElementById("withdrawalFlowRejectedCopy"),
  approvalRateValue: document.getElementById("withdrawalFlowApprovalRateValue"),
  approvalRateCopy: document.getElementById("withdrawalFlowApprovalRateCopy"),
  moncashValue: document.getElementById("withdrawalFlowMoncashValue"),
  moncashCopy: document.getElementById("withdrawalFlowMoncashCopy"),
  natcashValue: document.getElementById("withdrawalFlowNatcashValue"),
  natcashCopy: document.getElementById("withdrawalFlowNatcashCopy"),
  pendingValue: document.getElementById("withdrawalFlowPendingValue"),
  moncashShareValue: document.getElementById("withdrawalFlowMoncashShareValue"),
  natcashShareValue: document.getElementById("withdrawalFlowNatcashShareValue"),
  highlightsCopy: document.getElementById("withdrawalFlowHighlightsCopy"),
  definitionsCopy: document.getElementById("withdrawalFlowDefinitionsCopy"),
  approvedSvg: document.getElementById("withdrawalFlowApprovedSvg"),
  approvedAxis: document.getElementById("withdrawalFlowApprovedAxis"),
  approvedLastValue: document.getElementById("withdrawalFlowApprovedLastValue"),
  approvedLastCopy: document.getElementById("withdrawalFlowApprovedLastCopy"),
  approvedDeltaValue: document.getElementById("withdrawalFlowApprovedDeltaValue"),
  approvedDeltaCopy: document.getElementById("withdrawalFlowApprovedDeltaCopy"),
  approvedPeakValue: document.getElementById("withdrawalFlowApprovedPeakValue"),
  approvedPeakCopy: document.getElementById("withdrawalFlowApprovedPeakCopy"),
  cumulativeSvg: document.getElementById("withdrawalFlowCumulativeSvg"),
  cumulativeAxis: document.getElementById("withdrawalFlowCumulativeAxis"),
  cumulativeLastValue: document.getElementById("withdrawalFlowCumulativeLastValue"),
  cumulativeLastCopy: document.getElementById("withdrawalFlowCumulativeLastCopy"),
  cumulativeDeltaValue: document.getElementById("withdrawalFlowCumulativeDeltaValue"),
  cumulativeDeltaCopy: document.getElementById("withdrawalFlowCumulativeDeltaCopy"),
  cumulativePeakValue: document.getElementById("withdrawalFlowCumulativePeakValue"),
  cumulativePeakCopy: document.getElementById("withdrawalFlowCumulativePeakCopy"),
  methodsSvg: document.getElementById("withdrawalFlowMethodsSvg"),
  methodsAxis: document.getElementById("withdrawalFlowMethodsAxis"),
  rejectsSvg: document.getElementById("withdrawalFlowRejectsSvg"),
  rejectsAxis: document.getElementById("withdrawalFlowRejectsAxis"),
  tableBody: document.getElementById("withdrawalFlowTableBody"),
  tableNote: document.getElementById("withdrawalFlowTableNote"),
};
...
function renderSummary(snapshot = null) {
  ...
  dom.approvedCopy.textContent = `${formatInt(summary.approvedCount)} retraits ont ete approuves et sortent de l'entreprise.`;
  ...
  dom.definitionsCopy.textContent = `${String(defs.outflowRule || "")} ${String(defs.rejectionRule || "")}`.trim();

  dom.scanBadge.textContent = snapshot?.truncated
    ? `Source: withdrawals - tronque a ${formatInt(snapshot.scanLimit)} docs`
    : `Source: withdrawals - ${formatInt(snapshot.scannedWithdrawalDocs)} retraits lus`;
}
...
function renderLineChart(options = {}) {
  ...
  const gradientId = `withdrawalFlowGradient-${tone}`;
...
function renderBuckets(snapshot = null) {
  ...
  if (!buckets.length) {
    dom.tableBody.innerHTML = `<tr><td colspan="8" class="empty-copy">Aucun retrait trouve sur cette fenetre.</td></tr>`;
  ...
  dom.tableNote.textContent = snapshot?.truncated
    ? `Le snapshot a ete tronque apres ${formatInt(snapshot.scanLimit)} retraits lus pour proteger les couts Firestore.`
    : `${formatInt(snapshot.scannedWithdrawalDocs)} retraits de la periode ont ete lus directement depuis les sous-collections withdrawals.`;
}
...
function renderSnapshot(snapshot = null) {
  ...
  renderLineChart({
    ...
    tone: "emerald",
    emptyLabel: "Pas encore assez de retraits approuves sur cette periode pour tracer une courbe utile.",
  });
  renderChartInsights({
    points: snapshot?.series?.approvedHtg || [],
  ...
  renderLineChart({
    ...
    tone: "sky",
    emptyLabel: "La courbe cumulative apparaitra ici quand la periode contiendra des retraits approuves.",
  });
  renderChartInsights({
    points: snapshot?.series?.cumulativeApprovedHtg || [],
...
async function loadSnapshot() {
  ...
  setLoading(true);
  setStatus("Chargement des flux retraits...");
  try {
    const response = await getWithdrawalFlowAnalyticsSnapshotSecure(payload);
    const snapshot = response?.snapshot || null;
    renderSnapshot(snapshot);
    setStatus("Snapshot retraits charge.", "success");
  } catch (error) {
    console.error("[WITHDRAWAL_FLOW_V1] load failed", error);
    setStatus(error?.message || "Impossible de charger les analytics de retraits.", "error");
  } finally {
    setLoading(false);
  }
}
...
async function bootstrap() {
  const session = await ensureFinanceDashboardSession({
    title: "Flux retraits",
    description: "Connecte-toi avec le compte administrateur autorise pour consulter le flux des retraits.",
  });

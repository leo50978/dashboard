...
export const WITHDRAWAL_STATUS_META = {
  all: { label: "Tous les retraits", shortLabel: "Tous", accent: "#38bdf8" },
  pending: { label: "Retraits en attente", shortLabel: "En attente", accent: "#f59e0b" },
  approved: { label: "Retraits approuves", shortLabel: "Approuves", accent: "#22c55e" },
  rejected: { label: "Retraits rejetes", shortLabel: "Rejetes", accent: "#ef4444" },
};
...
function normalizeWithdrawal(docSnap) {
  ...
  return {
    ...
    customerPhone: String(data.customerPhone || data.phone || data.destinationValue || "").trim(),
    phone: String(data.phone || data.customerPhone || data.destinationValue || "").trim(),
    methodName: String(data.methodName || data.method || data.methodId || data.destinationType || "").trim(),
...
export async function loadClientWithdrawals(clientId = "") {
  ...
  try {
    ...
    console.error(`[WITHDRAWALS_DATA_V2] impossible de lire withdrawals pour ${normalizedClientId}`, error);
    return [];
  }
}
...
export async function loadWithdrawals(status = "all") {
  ...
  try {
    ...
    console.warn("[WITHDRAWALS_DATA_V2] fallback sans orderBy pour withdrawals", error);

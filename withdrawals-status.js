    ...
    <div data-modal-overlay="true" class="withdrawals-status-modal-overlay">
      <div class="withdrawals-status-modal-panel">
        <div class="withdrawals-status-modal-header">
          <div>
            <p style="margin:0;font-size:12px;text-transform:uppercase;letter-spacing:.18em;color:#64748b;font-weight:800;">Retrait</p>
            <h2 id="withdrawalsStatusModalTitle" style="margin:10px 0 0;font-size:clamp(1.6rem,3vw,2.5rem);line-height:1.08;">Details</h2>
          </div>
          <button type="button" id="withdrawalsStatusModalClose" style="border:none;background:#f8fafc;color:#0f172a;border-radius:999px;width:44px;height:44px;font-size:18px;font-weight:900;cursor:pointer;flex:0 0 auto;">×</button>
        </div>
        ...
        <div class="withdrawals-status-modal-footer">
          ...
          <button type="button" id="withdrawalsStatusModalDone" class="withdrawals-status-modal-btn" style="border:1px solid rgba(148,163,184,.35);background:white;color:#0f172a;">Fermer</button>
        </div>
      </div>
    </div>
  `;
    ...
    content.innerHTML = `
      <div class="withdrawals-status-modal-grid" style="margin-top:18px;">
        <div class="withdrawals-status-modal-card soft">
          ...
          <p style="margin:8px 0 0;color:#475569;">${escapeHtml(withdrawal.customerEmail || "-")}</p>
          <p style="margin:6px 0 0;color:#475569;">${escapeHtml(withdrawal.phone || "-")}</p>
        </div>
        <div class="withdrawals-status-modal-card soft">
          ...
          <p style="margin:8px 0 0;color:#475569;">${escapeHtml(withdrawal.methodName || "Methode non definie")}</p>
          <p style="margin:6px 0 0;"><span style="display:inline-flex;align-items:center;justify-content:center;border-radius:999px;background:rgba(15,23,42,.06);padding:6px 10px;font-size:12px;font-weight:800;color:#0f172a;">${escapeHtml(getStatusText(withdrawal.status))}</span></p>
        </div>
      </div>
      <div class="withdrawals-status-modal-grid withdrawals-status-modal-section">
        <div class="withdrawals-status-modal-card">
          ...
          <p style="margin:8px 0 0;color:#475569;">Soumis le ${escapeHtml(formatDate(withdrawal.createdAtMs))}</p>
          <p style="margin:8px 0 0;color:#475569;">Revise le ${escapeHtml(formatDate(withdrawal.reviewedAtMs))}</p>
        </div>
        <div class="withdrawals-status-modal-card">
          <p style="margin:0;font-size:12px;text-transform:uppercase;letter-spacing:.14em;color:#64748b;font-weight:800;">Note</p>
...
async function handleDecision(withdrawal, action) {
  ...
  try {
    ...
    console.error("[WITHDRAWALS_STATUS] decision failed", error);
    showToast(error?.message || "Erreur lors de la mise à jour du retrait.", "error");
  } finally {
    activeDecisionKey = "";
    setModalButtonsEnabled(true);
  }
...
async function handleDelete(withdrawal) {
  if (!withdrawal) return;
  const confirmed = window.confirm("Supprimer définitivement ce retrait ?");
  if (!confirmed) return;
  ...
  try {
    ...
    console.error("[WITHDRAWALS_STATUS] delete failed", error);
        ...
        <tr>
          ...
          <td data-label="Actions" class="actions-cell">
            <div class="row-actions">
              <button type="button" data-action="view" data-withdrawal-id="${escapeHtml(item.id)}" data-client-id="${escapeHtml(item.clientId)}" style="border:1px solid rgba(148,163,184,.26);background:white;color:#0f172a;border-radius:999px;padding:8px 12px;font-size:12px;font-weight:800;cursor:pointer;">Consulter</button>
              ${canReview && canAct ? `
      ...
      <tr class="day-group-row">
        <td class="day-group-cell" colspan="8" style="padding:18px 12px 10px;border-bottom:none;background:rgba(15,23,42,.03);">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;">
            ...
            <span style="font-size:12px;font-weight:800;color:#64748b;">${group.rows.length} retrait(s)</span>
          </div>
        </td>
      </tr>
      ${sectionRows}
...
async function init() {
  try {
    ...
    console.error("[WITHDRAWALS_STATUS] init failed", error);
    loadingEl?.classList.add("hidden");
    if (errorEl) {

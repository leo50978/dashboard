...
function buildPayload() {
  ...
  const payload = {
    ...
    stakeHtg: safeInt(dom.stake?.value || 0),
  };
  if (windowKey === "custom") {
    return {
      ...payload,
...
function renderSummary(snapshot = {}) {
  ...
  const avgStakeHtg = safeInt(summary.avgStakeHtg || Math.round(safeFloat(summary.avgStakeDoes) / RATE_HTG_TO_DOES));
  if (dom.avgStake) dom.avgStake.textContent = formatHtg(avgStakeHtg);
  ...
  if (dom.stakeMix) {
    ...
    dom.stakeMix.textContent = stakeMix.length > 0
      ? stakeMix.map((item) => {
          const stakeHtg = safeInt(item.stakeHtg || Math.round(safeFloat(item.stakeDoes) / RATE_HTG_TO_DOES));
          const label = item.labelHtg || formatHtg(stakeHtg);
          return `${formatInt(item.count)} en ${label}`;
        }).join(" • ")
      : "Aucune salle Morpion sur la periode.";
  }
...
function renderCharts(snapshot = {}) {
  ...
  if (stakeCtx) {
    chartState.stakes = new ChartLib(stakeCtx, {
      type: "bar",
      data: {
        labels: stakeMix.map((item) => item.labelHtg || formatHtg(item.stakeHtg || Math.round(safeFloat(item.stakeDoes) / RATE_HTG_TO_DOES))),
        datasets: [{
          label: "Salles",
...
async function refreshMorpionAnalytics() {
  try {
    ...
    await ensureFinanceDashboardSession({
      title: "Analytics Morpion V2",
      description: "Connecte-toi avec le compte administrateur autorise pour consulter les performances du mode Morpion.",
    });

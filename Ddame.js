...
import { getDameAnalyticsSnapshotSecure } from "./secure-functions.js";
...
const dom = {
  status: document.getElementById("dameStatus"),
  refreshBtn: document.getElementById("dameRefreshBtn"),
  windowSelect: document.getElementById("dameWindow"),
  dateFrom: document.getElementById("dameDateFrom"),
  dateTo: document.getElementById("dameDateTo"),
  composition: document.getElementById("dameComposition"),
  winnerType: document.getElementById("dameWinnerType"),
  stake: document.getElementById("dameStake"),
  coverage: document.getElementById("dameCoverage"),
  generatedAt: document.getElementById("dameGeneratedAt"),
  totalMatches: document.getElementById("dameTotalMatches"),
  humanOnlyMatches: document.getElementById("dameHumanOnlyMatches"),
  withBots: document.getElementById("dameWithBots"),
  botWins: document.getElementById("dameBotWins"),
  humanWins: document.getElementById("dameHumanWins"),
  totalMatchesNote: document.getElementById("dameTotalMatchesNote"),
  humanOnlyMatchesNote: document.getElementById("dameHumanOnlyMatchesNote"),
  withBotsNote: document.getElementById("dameWithBotsNote"),
  botWinsNote: document.getElementById("dameBotWinsNote"),
  humanWinsNote: document.getElementById("dameHumanWinsNote"),
  avgDuration: document.getElementById("dameAvgDuration"),
  avgStake: document.getElementById("dameAvgStake"),
  humanOnlyShare: document.getElementById("dameHumanOnlyShare"),
  withBotShare: document.getElementById("dameWithBotShare"),
  stakeMix: document.getElementById("dameStakeMix"),
};
...
function renderSummary(snapshot = {}) {
  ...
  if (dom.humanOnlyMatchesNote) dom.humanOnlyMatchesNote.textContent = `${formatPercent(summary.humanOnlyRatePct)} du trafic Dame`;
  if (dom.withBotsNote) dom.withBotsNote.textContent = `${formatPercent(summary.withBotRatePct)} des salles avec bot (attendu: 0%)`;
  if (dom.botWinsNote) dom.botWinsNote.textContent = `${formatPercent(summary.botWinRatePct)} des matchs gagnés par un bot`;
  if (dom.humanWinsNote) dom.humanWinsNote.textContent = `${formatPercent(summary.humanWinRatePct)} des matchs gagnés par un humain`;
  ...
  if (dom.stakeMix) {
    ...
    dom.stakeMix.textContent = stakeMix.length > 0
      ...
      : "Aucune salle Dame sur la période.";
  }
...
function renderCharts(snapshot = {}) {
  ...
  const trendCtx = document.getElementById("dameMatchesTrendChart");
  const outcomeCtx = document.getElementById("dameOutcomeChart");
  const compositionCtx = document.getElementById("dameCompositionChart");
  const stakeCtx = document.getElementById("dameStakeChart");
...
async function refreshDameAnalytics() {
  try {
    setStatus("Chargement des analytics Dame...", "neutral");
    await ensureFinanceDashboardSession({
      title: "Analytics Jeu de Dame",
      description: "Connecte-toi avec le compte administrateur autorisé pour consulter les performances du mode Dame.",
    });
    const result = await getDameAnalyticsSnapshotSecure(buildPayload());
    const snapshot = result?.snapshot || {};
    renderSummary(snapshot);
    renderCharts(snapshot);
    setStatus("Analytics Dame mises a jour.", "success");
  } catch (error) {
    console.error("[MORPION_DASHBOARD] refresh error", error);
    setStatus(error?.message || "Impossible de charger les analytics Dame.", "error");
  }
}

function bindEvents() {
  dom.refreshBtn?.addEventListener("click", refreshDameAnalytics);
  dom.windowSelect?.addEventListener("change", () => {
    syncDatesForWindow(dom.windowSelect.value);
    void refreshDameAnalytics();
  });
  ...
  dom.composition?.addEventListener("change", () => void refreshDameAnalytics());
  dom.winnerType?.addEventListener("change", () => void refreshDameAnalytics());
  dom.stake?.addEventListener("change", () => void refreshDameAnalytics());
}

async function bootstrap() {
  syncDatesForWindow(String(dom.windowSelect?.value || "30d"));
  bindEvents();
  await refreshDameAnalytics();
}

    ...
    <article class="result-card">
      <div class="result-head">
        ...
        <div style="text-align:right;">
          <div style="font-size:0.82rem;color:${client.accountFrozen ? "#ff9bab" : "#9fd8ff"};font-weight:700;">
            ${client.accountFrozen ? "Compte gelé" : "Compte actif"}
          </div>
      ...
      <div class="result-grid">
        ...
        <div>
          ...
          <strong>${client.hasApprovedDeposit ? "Déposant" : "Sans dépôt approuvé"}</strong>
        </div>
      </div>
...
async function searchClients() {
  ...
  try {
    ...
    renderResults();
    setStatus(
      state.results.length ? `${formatInt(state.results.length)} client(s) trouvé(s).` : "Aucun client trouvé.",
...
async function openResetModal(clientId = "") {
  ...
  try {
    ...
    dom.modalCopy.textContent = `UID ${client.uid || client.id || "-"} · ${buildClientSubtitle(client)} · HTG ${formatHtg(client.htgBalance)} · Does ${formatDoes(client.doesBalance)}.`;

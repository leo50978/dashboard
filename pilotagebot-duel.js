...
function formatDrawdown(htgValue, pctValue) {
  ...
  return `${drawdown > 0 ? "-" : ""}${formatInt(drawdown)} HTG Â· ${formatPercent(pctValue)}`;
}
...
function renderEquityCurve(snapshot = null) {
  ...
  if (points.length < 2) {
    ...
    dom.recoveryCopy.textContent = "Le moteur affichera ici le sommet precedent, le drawdown et la reprise du Duel quand le volume sera suffisant.";
    return;
  }
  ...
  if (drawdownHtg <= 0) {
    dom.recoveryCopy.textContent = `La courbe Duel tient actuellement son sommet sur la fenetre ${state.window}. Le systeme peut respirer sans trop charger les bots.`;
    return;
  }
  dom.recoveryCopy.textContent = `La courbe Duel reste sous son dernier sommet de ${formatDrawdown(snapshot?.drawdownHtg, snapshot?.drawdownPct)}. Dernier pic atteint le ${formatDateTime(snapshot?.lastPeakAtMs)}. Tant que ce drawdown reste ouvert, le pilotage auto garde plus de pression.`;
}
...
function renderStakeMix(snapshot = null) {
  ...
  if (!rows.length) {
    dom.stakeMixGrid.innerHTML = `<p class="empty-copy">Le mix par mise apparaitra ici quand les Duels seront archives.</p>`;
    return;
  }
...
function renderSnapshot() {
  ...
  dom.netCopy.textContent = `Encaisse ${formatHtg(snapshot.collectedHtg)} Â· payout ${formatHtg(snapshot.payoutHtg)}.`;
...
async function bootstrap() {
  const user = await ensureFinanceDashboardSession({
    title: "Pilotage Duel",
    subtitle: "Connecte-toi avec l'admin finance pour piloter les bots du Duel.",
  });

...
const getDepositMethodAnalyticsSnapshotSecure = typeof secureFunctions.getDepositMethodAnalyticsSnapshotSecure === "function"
  ? secureFunctions.getDepositMethodAnalyticsSnapshotSecure
  : async (payload = {}) => {
      const callable = httpsCallable(firebaseFunctions, "getDepositMethodAnalyticsSnapshot");
...
function syncWindowUi(snapshot = null) {
  ...
  dom.windowBadge.textContent = `Fenêtre ${rangeDays} jour${rangeDays > 1 ? "s" : ""} · ${granularity === "hour" ? "horaire" : granularity === "week" ? "hebdomadaire" : "journalière"}`;
}
...
function renderSummary(snapshot = null) {
  ...
  dom.requestedCopy.textContent = `${formatInt(summary.requestedCount)} demandes réelles ont été lues sur cette fenêtre.`;

  dom.approvedValue.textContent = formatHtg(summary.approvedHtg);
  dom.approvedCopy.textContent = `${formatInt(summary.approvedCount)} dépôts ont été approuvés et entrent dans l’entreprise.`;

  dom.rejectedValue.textContent = formatHtg(summary.rejectedHtg);
  dom.rejectedCopy.textContent = `${formatInt(summary.rejectedCount)} demandes ont été rejetées sur cette fenêtre.`;

  dom.approvalRateValue.textContent = formatPercent(summary.approvedRatePct);
  dom.approvalRateCopy.textContent = `${formatPercent(summary.rejectedRatePct)} des montants demandés ont fini rejetés.`;

  dom.moncashValue.textContent = formatHtg(summary.moncashApprovedHtg);
  dom.moncashCopy.textContent = `${formatInt(summary.moncashApprovedCount)} validations MonCash, soit ${formatPercent(summary.moncashApprovedSharePct)} du flux approuvé.`;

  dom.natcashValue.textContent = formatHtg(summary.natcashApprovedHtg);
  dom.natcashCopy.textContent = `${formatInt(summary.natcashApprovedCount)} validations NatCash, soit ${formatPercent(summary.natcashApprovedSharePct)} du flux approuvé.`;
  ...
  dom.highlightsCopy.textContent = `${formatHtg(summary.approvedHtg)} ont été approuvés sur la fenêtre, contre ${formatHtg(summary.rejectedHtg)} rejetés. MonCash pèse ${formatPercent(summary.moncashApprovedSharePct)} du flux validé et NatCash ${formatPercent(summary.natcashApprovedSharePct)}.`;
  ...
  dom.scanBadge.textContent = snapshot?.truncated
    ? `Source: orders · tronqué à ${formatInt(snapshot.scanLimit)} docs`
    : `Source: orders · ${formatInt(snapshot.scannedOrderDocs)} dépôts lus`;
}
...
function renderBuckets(snapshot = null) {
  ...
  if (!buckets.length) {
    dom.tableBody.innerHTML = `<tr><td colspan="8" class="empty-copy">Aucun dépôt trouvé sur cette fenêtre.</td></tr>`;
    dom.tableNote.textContent = "Change la période si tu veux analyser un autre flux.";
    return;
  }
  ...
  dom.tableNote.textContent = snapshot?.truncated
    ? `Le snapshot a été tronqué après ${formatInt(snapshot.scanLimit)} dépôts lus pour protéger les coûts Firestore.`
    : `${formatInt(snapshot.scannedOrderDocs)} dépôts de la période ont été lus directement depuis les sous-collections orders.`;
}
...
function renderSnapshot(snapshot = null) {
  ...
  renderLineChart({
    ...
    tone: "emerald",
    emptyLabel: "Pas encore assez de dépôts approuvés sur cette période pour tracer une courbe utile.",
  });
  renderChartInsights({
    points: snapshot?.series?.approvedHtg || [],
  ...
  renderLineChart({
    ...
    tone: "sky",
    emptyLabel: "La courbe cumulative apparaîtra ici quand la période contiendra des dépôts approuvés.",
  });
  renderChartInsights({
    points: snapshot?.series?.cumulativeApprovedHtg || [],
  ...
  renderDualLineChart({
    ...
    emptyLabel: "La comparaison MonCash/NatCash apparaîtra ici.",
  });
  ...
  renderDualLineChart({
    ...
    emptyLabel: "La comparaison demandé/rejeté apparaîtra ici.",
  });
...
async function bootstrap() {
  const session = await ensureFinanceDashboardSession({
    ...
    requiredAdminEmail: "leovitch2004@gmail.com",
  });

  ...
  dom.results.innerHTML = results.map((client) => `
    <article class="result-card">
      ...
      <div class="result-grid">
        ...
        <div>
          <span>HTG jouables</span>
          <strong>${formatHtg(client.playableHtg)}</strong>
        </div>
        <div>
  ...
  dom.historyList.innerHTML = recentOrders.map((order) => `
    <li class="history-item">
      ...
      <div style="margin-top:8px;font-size:0.86rem;color:#9db0d5;">
        ...
        ${safeInt(order.bonusHtgAwarded) > 0 ? ` · bonus ${formatHtg(order.bonusHtgAwarded)}` : ""}
      </div>
    </li>
  `).join("");
}
...
async function openCreditModal(clientId = "") {
  ...
  try {
    ...
    dom.modalCopy.textContent = `UID ${client.uid || client.id || "-"} · ${buildClientSubtitle(client)} · HTG approuvés ${formatHtg(client.approvedHtgAvailable)} · HTG provisoires ${formatHtg(client.provisionalHtgAvailable)} · HTG jouables ${formatHtg(client.playableHtg)}.`;
...
async function submitAgentCredit(event) {
  ...
  try {
    ...
    const bonusCopy = safeInt(response?.bonusHtgAwarded) > 0
      ? ` Bonus automatique: ${formatHtg(response.bonusHtgAwarded)}.`
      : "";

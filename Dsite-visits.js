...
function renderSummary(snapshot = {}, result = {}) {
  ...
  if (dom.rangeVisitsNote) dom.rangeVisitsNote.textContent = `${formatInt(summary.activeBuckets)} point(s) sur la periode filtree`;
  if (dom.allTimeVisitsNote) dom.allTimeVisitsNote.textContent = "Compteur historique toutes sessions confondues";
  if (dom.todayVisitsNote) dom.todayVisitsNote.textContent = "Visites enregistrees depuis 00h aujourd'hui";
  if (dom.peakVisitsNote) dom.peakVisitsNote.textContent = summary.peakBucketLabel || "Pas encore de pic releve";
...
async function refreshSiteVisits() {
  try {
    ...
    await ensureFinanceDashboardSession({
      ...
      description: "Connecte-toi avec le compte administrateur autorise pour consulter les visites globales du site.",
    });
    const result = await getSiteVisitsAnalyticsSnapshotSecure(buildPayload());
    const snapshot = result?.snapshot || {};
    renderSummary(snapshot, result || {});
    renderCharts(snapshot);
    setStatus("Analytics de visites a jour.", "success");
  } catch (error) {
    console.error("[SITE_VISITS_DASHBOARD_V2] refresh error", error);
